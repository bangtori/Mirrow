...
export default function ResultBodySection({ resultModel, responseCount }: ResultBodySectionProps) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-8 py-10">
            ...
            <JohariPanel type="hidden" words={hidden.map((word) => ({ word: word.korean, percent: null }))} />
            <JohariPanel type="unknown" words={unknown.map((word) => ({ word: word.korean, percent: null }))} locked={responseCount < 5} />
        </div>
    )
}
